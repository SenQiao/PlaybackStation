...
//04/04/2017 ????
        // $('#play-results').click(function(evt) {
            if ($('#play-results').hasClass('play-result')) {
...
//04/04/2017
    ...
    function processCollTypes()
    {
        ...
        _.forEach(collDefs, function(theCollDef, defIndex) {
            ...
            $(htmlEntry).insertBefore("#no-search-boxes");
        });
        $('<div class="play-option" data-index="'+collTypes.length+
            '" data-coll-type="search"><i class="play-icon fa fa-search"></i> Search</div>').insertBefore("#no-search-boxes");
